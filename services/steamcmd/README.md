...
Headless management of Wallpaper Engine (app id 431960) using the [cm2network/steamcmd](https://hub.docker.com/r/cm2network/steamcmd) image, replacing the Steam desktop client. Downloads follow the official Steam library layout under `~/.local/share/wallpaper_engine`, so waywallen's WE plugin can consume them directly — just point it at that directory.
...
## Download / Update a single wallpaper via ID（profile: wallpaper）
...
## Sync subscriptions and Wallpaper Engine App（profile: sync）
...
The command is inlined in the compose file (`command:`); the username defaults to `${STEAM_USER:-cloudmapleleaf}` — switch accounts with `STEAM_USER=xxx docker compose --profile sync up`. The container exits on its own when done, and leaving it around is fine — the next `up` recreates it.
...
One manual step is needed after the first sync: steamcmd places the WE assets deep in the library tree, but waywallen's plugin looks for them directly under the library root. Create a symlink to bridge the two:

```bash
ln -s ../../../assets ~/.local/share/wallpaper_engine/steamapps/common/wallpaper_engine/assets
```

## Descriptions

- The first login asks for your password and a Steam Guard code; both are cached in the `steam-home` volume afterwards, so subsequent runs need no interaction at all.
